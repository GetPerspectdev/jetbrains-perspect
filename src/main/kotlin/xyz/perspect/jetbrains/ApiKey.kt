...
class ApiKey(project: Project?) : DialogWrapper(project, true) {
    ...
    init {
        ...
        label = JLabel("Enter your Perspect API key:", JLabel.CENTER)
        panel.add(label)
        input = JTextField(36)
        panel.add(input)
        link = LinkPane("https://app.perspect.xyz/account")
        panel.add(link)
    ...
    override fun doValidate(): ValidationInfo? {
        ...
        return try {
            UUID.fromString(apiKey.replaceFirst("^perspect_", ""))
            null
        } catch (e: Exception) {
