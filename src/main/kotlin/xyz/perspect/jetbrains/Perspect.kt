...
class Perspect : ApplicationComponent {
    ...
    companion object {
        ...
        private fun checkDebug() {
            ...
            ApplicationManager.getApplication().invokeLater {
                Messages.showWarningDialog("Your IDE may respond slower. Disable debug mode from Tools -> Perspect Settings.", "Perspect Debug Mode Enabled")
            }
        }

        fun checkApiKey() {
            if (Perspect.cancelApiKey) return
            ApplicationManager.getApplication().invokeLater {
        ...
        fun appendHeartbeat(file: VirtualFile, project: Project?, isWrite: Boolean, lineStats: LineStats?) {
            ...
            if (READY) {
                ...
                if (project != null) {
                    ...
                    statusbar?.updateWidget("Perspect")
                }
            }
            ...
            ApplicationManager.getApplication().executeOnPooledThread {
                val h = Heartbeat().apply {
                    ...
                    isBuilding = Perspect.isBuilding
                    lineStats?.let {
        ...
        private fun sendHeartbeat(heartbeat: Heartbeat, extraHeartbeats: ArrayList<Heartbeat>) {
            ...
            try {
                ...
                if (Dependencies.isWindows() && e.toString().contains("Access is denied")) {
                    try {
                        Messages.showWarningDialog("Microsoft Defender is blocking Perspect. Please allow ${Dependencies.getCLILocation()} to run so Perspect can upload code stats to your dashboard.", "Error")
                    } catch (ex: Exception) { }
                }
            }
        }
        ...
        private fun pluginString(): String? {
            ...
            return "$IDE_NAME/$IDE_VERSION $IDE_NAME-perspect/$VERSION"
        }

        private fun getBuiltinProxy(): String? {
            val config = HttpConfigurable.getInstance()

            if (!config.isHttpProxyEnabledForUrl("https://loom.getperspect.dev")) return null
        ...
        fun openDashboardWebsite() {
            BrowserUtil.browse("https://app.perspect.xyz/")
        }
    ...
    override fun initComponent() {
        VERSION = try {
            ...
            PluginManager.getPlugin(PluginId.getId("xyz.perspect.jetbrains"))?.version ?: ""
        } catch (e: Exception) {
            // use PluginManagerCore if PluginManager deprecated
            PluginManagerCore.getPlugin(PluginId.getId("xyz.perspect.jetbrains"))?.version ?: ""
        }
        log.info("Initializing Perspect plugin v$VERSION (https://perspect.xyz/)")
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            if (!Dependencies.isCLIInstalled()) {
                ...
                if (System.getenv("WAKATIME_CLI_LOCATION") != null && !System.getenv("WAKATIME_CLI_LOCATION").trim().isEmpty()) {
                    val perspectCLI = File(System.getenv("WAKATIME_CLI_LOCATION"))
                    if (perspectCLI.exists()) {
    ...
    private fun setupEventListeners() {
        ApplicationManager.getApplication().invokeLater {
            val disposable = Disposer.newDisposable("PerspectListener")
