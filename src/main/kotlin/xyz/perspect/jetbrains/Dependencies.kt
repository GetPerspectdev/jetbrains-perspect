...
object Dependencies {
    ...
    fun getResourcesLocation(): String {
        ...
        val resourcesFolder = File(userHomeDir, ".perspect")

        if (System.getenv("PERSPECT_HOME") != null && System.getenv("PERSPECT_HOME").trim().isNotEmpty()) {
            resourcesFolder.takeIf { it.exists() }?.let {
                resourcesLocation = it.absolutePath
                Perspect.log.debug("Using \$PERSPECT_HOME for resources folder: $resourcesLocation")
    ...
    fun isCLIOld(): Boolean {
        ...
        try {
            ...
            Perspect.log.debug("perspect-cli local version output: \"$output\"")
            Perspect.log.debug("perspect-cli local version exit code: ${p.exitValue()}")

            if (p.exitValue() != 0) return true

            // disable updating perspect-cli when it was built from source
            if (output.trim() == "<local-build>") {
                return false
            }

            val accessed = ConfigFile.get("internal", "cli_version_last_accessed", true)
            val now = Perspect.getCurrentTimestamp().toBigInteger()
            accessed?.let {
                try {
                    ...
                    if (lastAccessed != null && lastAccessed.add(fourHours).compareTo(now) > 0) {
                        Perspect.log.debug("Skip checking for perspect-cli updates because recently checked ${now.subtract(lastAccessed).toString()} seconds ago")
                        return false
                    }
                } catch (e2: NumberFormatException) {
                    Perspect.warnException(e2)
                }
            }

            val cliVersion = latestCliVersion()
            Perspect.log.debug("Latest perspect-cli version: $cliVersion")
            if (output.trim() == cliVersion) return false
        } catch (e: Exception) {
            Perspect.warnException(e)
        }
        return true
    }
        ...
        System.getenv("WAKATIME_CLI_LOCATION")?.let {
            if (it.trim().isNotEmpty()) {
                ...
                if (wakatimeCLI.exists()) {
                    Perspect.log.debug("Using \$WAKATIME_CLI_LOCATION as CLI Executable: $wakatimeCLI")
                    return it
                }
            }
        }
    ...
    fun installCLI() {
        ...
        val zipFile = combinePaths(getResourcesLocation(), "perspect-cli.zip")

        if (zipFile?.let { downloadFile(url, it) } == true) {
            // Delete old perspect-cli if it exists
    ...
    private fun reportMissingPlatformSupport(osname: String, architecture: String) {
        val url = "https://loom.getperspect.dev/api/v1/cli-missing?osname=$osname&architecture=$architecture&plugin=${Perspect.IDE_NAME}"
        try {
    ...
    fun downloadFile(url: String, saveAs: String): Boolean {
        ...
        val downloadUrl = try {
            ...
            Perspect.log.error("DownloadFile($url) failed to init new URL")
            Perspect.log.error(e)
            return false
        }

        Perspect.log.debug("DownloadFile(${downloadUrl})")
        ...
        try {
            ...
            try {
                ...
                (downloadUrl.openConnection() as HttpsURLConnection).apply {
                    ...
                    setRequestProperty("User-Agent", "github.com/getperspectdev/jetbrains-perspect")
    ...
    fun getUrlAsString(url: String, lastModified: String?, updateLastModified: Boolean): Response? {
        ...
        val downloadUrl = try {
            ...
            Perspect.log.error("getUrlAsString($url) failed to init new URL")
            Perspect.log.error(e)
            return null
        }

        Perspect.log.debug("getUrlAsString(${downloadUrl})")
    ...
    private fun setupProxy() {
        ...
        if (!proxyConfig.isNullOrBlank()) {
            ...
            try {
                ...
                Perspect.log.error("Proxy string must follow https://user:pass@host:port format: $proxyConfig")
                Perspect.errorException(e)
            }
        }
    }
